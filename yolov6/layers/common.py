...
class VSS_Out(nn.Module):
    def __init__(self, out_channels, idx):
        super(VSS_Out, self).__init__()
        self.idx = idx
        self.out_channels = out_channels
...
class iRMB(nn.Module):
    def __init__(self, dim_in, dim_out, norm_in=True, has_skip=True, exp_ratio=2,
        ...
        if self.attn_s:
            ...
            self.qk = nn.Sequential(
                nn.Conv2d(dim_in, int(dim_in * 2), 1, bias=qkv_bias),
                nn.BatchNorm2d(int(dim_in * 2)))
            ...
            if v_proj:
                # self.v = nn.Sequential(nn.ReLU6(), nn.Conv2d(dim_in, dim_mid, kernel_size=1, groups=self.num_head if v_group else 1, bias=qkv_bias))
                self.v = nn.Sequential(
                    nn.Conv2d(dim_in, dim_mid, kernel_size=1, groups=self.num_head if v_group else 1, bias=qkv_bias),
                    self.act
                )
                ...
                self.v = nn.Identity()
        # self.conv_local =  UniRepLKNetBlock(dim_mid, kernel_size=kersize)
        self.conv_local = nn.Sequential(UniRepLKNetBlock(dim_mid, kernel_size = 5), nn.SiLU())
         # Conv(dim_mid, dim_mid, kernel_size=dw_ks, stride=stride, groups=dim_mid)
        self.se = SEAttention(dim_mid, reduction=se_ratio) if se_ratio > 0.0 else nn.Identity()

        self.proj_drop = nn.Dropout(drop)
        self.proj = nn.Sequential(nn.ReLU6(), nn.Conv2d(dim_mid, dim_out, kernel_size=1))
    ...
    def forward(self, x):
        ...
        # x = list(x.split((self.dim_out, self.dim_out, self.dim_out), 1))
...
class HighAttentionConcat(nn.Module):
    def __init__(self, out_channel, pre_channel):
        super(HighAttentionConcat, self).__init__()
        self.med_channel = out_channel
        self.low_channel = out_channel // 2
        self.pre_channel = pre_channel
        self.pre_big_channel = pre_channel * 2
        self.conv_med1 = Conv(self.med_channel, self.med_channel, 1, 1)
        self.conv_med2 = Conv(self.med_channel, self.med_channel, 1, 1)
        self.conv_high = Conv(self.pre_big_channel, self.med_channel, 1, 1)
        # self.down = Conv(self.pre_channel, self.low_channel, 3, 2)
        # self.up = nn.Upsample(None, 2, 'nearest')
        # self.up_conv = Conv(up_channel, 2 * out_channel, 1, 1)
        self.act = nn.Sigmoid()
        self.cv1 =Conv(2, 1, 3, 1)
        self.act2 = nn.Sigmoid()
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.fc = Conv(self.med_channel, self.med_channel, 1, 1)
    ...
    def forward(self, x):
        ...
        med_x = x[2]
        high_x = x[3]
        low_x = torch.cat([low_x1, low_x2], 1)

        # low_x = self.down(low_x)
        med_x1 = self.conv_med1(med_x)
        med_x2 = self.conv_med2(med_x)
        high_x = self.conv_high(high_x)
        # high_x = self.up(high_x)


        # med_x_list = list(med_x.split((self.low_channel, self.low_channel), 1))
        mean_sa = torch.mean(med_x1, 1, keepdim=True)
        max_sa = torch.max(med_x1, 1, keepdim=True)[0]
        med_x_sa = self.act(self.cv1(torch.cat([mean_sa, max_sa], 1)))

        low_x_sa = med_x_sa * low_x
        # med_x_sa_out = med_x1 + med_x_sa  #64

        high_x_ca = self.act2(self.fc(self.pool(high_x)))
        # high_x_ca = self.act2(high_x_list[0])
        high_x_sa_ca_out = high_x_ca * med_x2

        out = torch.cat([low_x_sa, high_x_sa_ca_out, med_x1], 1)
        # high_x_sa_ca_out = high_x_list[1] + high_x_ca

        return out
class HighAttentionConcat_simple(nn.Module):
    def __init__(self, out_channel, pre_channel):
        super(HighAttentionConcat_simple, self).__init__()
        self.med_channel = out_channel
        self.low_channel = out_channel // 2
        self.pre_channel = pre_channel
        self.pre_big_channel = pre_channel * 2
    ...
    def forward(self, x):
        ...
        return out
class LowAttentionConcat0(nn.Module):
    def __init__(self, inchannels, out_channel):
        super(LowAttentionConcat0, self).__init__()
        self.med_channel = out_channel
        self.low_channel = out_channel // 2
        self.pre_channel = pre_channel
        self.pre_big_channel = pre_channel * 2
        ...
        self.act2 = nn.Sigmoid()
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.fc = Conv(self.low_channel, self.low_channel, 1, 1)
    ...
    def forward(self, x):
        ...
        high_x_list = list(high_x.split((self.low_channel, self.pre_big_channel - self.low_channel), 1))
        high_x_ca = self.act2(self.fc(self.pool(high_x_list[0])))
        # high_x_ca = self.act2(high_x_list[0])
        high_x_ca = high_x_ca * med_x

        out = torch.cat([high_x_ca, high_x_list[1]], 1)
        # high_x_sa_ca_out = high_x_list[1] + high_x_ca
...
#             self.med_channel_in = in_channel[1]
#             self.low_channel_in = in_channel[0]
#             self.med_channel_out = out_channel
#             self.low_channel_out = out_channel // 2
#             self.conv_med1 = Conv(self.med_channel_in, self.med_channel_out)
#             self.conv_med2 = Conv(self.med_channel_in, self.low_channel_out)
#             self.conv_low = Conv(self.low_channel_in, self.low_channel_out)
#             self.act = nn.Sigmoid()
#             self.cv1 = Conv(2, 1, 3, 1)
#             self.act2 = nn.Sigmoid()
#             self.pool = nn.AdaptiveAvgPool2d(1)
#             self.fc = nn.Conv2d(self.med_channel_out, self.med_channel_out, 1)
#         elif self.in_numbers == 3:
#             self.high_channel_in = in_channel[2]
#             self.low_channel_in = in_channel[0]
#             self.med_channel_in = in_channel[1]
#             self.med_channel_out = out_channel
#             self.low_channel_out = out_channel // 2
#             self.conv_med1 = Conv(self.med_channel_in + self.high_channel_in, self.med_channel_out)
#             self.conv_med2 = Conv(self.med_channel_in + self.high_channel_in, self.low_channel_out)
#             self.conv_low = Conv(self.low_channel_in, self.low_channel_out)
#             self.act = nn.Sigmoid()
#             self.cv1 = Conv(2, 1, 3, 1)
#             self.act2 = nn.Sigmoid()
...
#             self.fc = nn.Conv2d(self.med_channel_out, self.med_channel_out, 1)
#
#             # self.out = Conv(self.low_channel_out + self.med_channel_out, out_channel)
#
#
#
#     def forward(self, x):
...
#             med_x1 = self.conv_med1(med_x)
#             med_x2 = self.conv_med2(med_x)
#             mean_sa = torch.mean(med_x2, 1, keepdim=True)
#             max_sa = torch.max(med_x2, 1, keepdim=True)[0]
#             med_x_sa = self.act(self.cv1(torch.cat([mean_sa, max_sa], 1)))
#             low_x_sa = med_x_sa * low_x
#             high_x_ca = self.act2(self.fc(self.pool(med_x1)))
#             high_x_ca = high_x_ca * med_x1
#             out = torch.cat([low_x, low_x_sa, high_x_ca], 1)
#         elif self.in_numbers == 3:
#             low_x = x[0]
#             med_x = x[1]
#             high_x = x[2]
#             low_x = self.conv_low(low_x)
#             med_x = torch.cat([high_x, med_x], 1)
#             med_x1 = self.conv_med1(med_x)
#             med_x2 = self.conv_med2(med_x)
#             mean_sa = torch.mean(med_x2, 1, keepdim=True)
#             max_sa = torch.max(med_x2, 1, keepdim=True)[0]
...
#         # # med_x_sa_out = med_x1 + med_x_sa  #64
#         #
#         # high_x_list = list(high_x.split((self.med_channel, self.med_channel), 1))
#         # high_x_ca = self.act2(self.fc(self.pool(high_x_list[0])))
#         # # high_x_ca = self.act2(high_x_list[0])
#         # high_x_ca = high_x_ca * med_x2
#         # high_x_sa_ca_out = high_x_ca + high_x_list[1]
#         #
#         # out = torch.cat([low_x_sa, high_x_sa_ca_out, med_x1], 1)
#         # # high_x_sa_ca_out = high_x_list[1] + high_x_ca
#
...
class LAC(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        elif self.in_numbers == 4:
            self.high_channel_in = in_channel[3]
            self.low_channel_in = in_channel[0]
            self.low_channel_in_up = in_channel[1]
            self.med_channel_in = in_channel[2]
            self.med_channel_out = out_channel
            self.low_channel_out = out_channel // 2
            self.conv_med1 = Conv(self.med_channel_in + self.high_channel_in, self.med_channel_out)
            self.conv_med2 = Conv(self.med_channel_in + self.high_channel_in, self.low_channel_out)
            self.conv_low = Conv(self.low_channel_in + self.low_channel_in_up, self.low_channel_out)
            self.act = nn.Sigmoid()
            self.cv1 = Conv(2, 1, 3, 1)
            self.act2 = nn.Sigmoid()
            self.pool = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, x):
        if self.in_numbers == 2:
            ...
            high_x_ca = self.act2(self.fc(self.pool(med_x1)))
            high_x_ca = high_x_ca * med_x1
            out = torch.cat([low_x_sa, high_x_ca], 1)
        elif self.in_numbers == 3:
            low_x = x[0]
            med_x = x[1]
            high_x = x[2]
            ...
            med_x = x[2]
            high_x = x[3]
            low_x = torch.cat([low_x, low_x_up], 1)
            low_x = self.conv_low(low_x)
            med_x = torch.cat([high_x, med_x], 1)
            med_x1 = self.conv_med1(med_x)
            med_x2 = self.conv_med2(med_x)
...
class HAC(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        if self.in_numbers == 2:
            ...
            self.low_channel_in = in_channel[0]
            self.med_channel_out = out_channel
            self.low_channel_out = out_channel
            self.conv_med1 = Conv(self.med_channel_in, self.med_channel_out)
            self.conv_med2 = Conv(self.med_channel_in, self.low_channel_out)
            self.conv_low = Conv(self.low_channel_in, self.low_channel_out)
            self.act = nn.Sigmoid()
            ...
            self.small_channel_in = in_channel[0]
            self.small_channel_in2 = in_channel[1]
            self.big_channel_in = in_channel[2]
            self.med_channel_in = in_channel[3]
            self.med_channel_out = out_channel
            self.low_channel_out = out_channel
            self.conv_med1 = Conv(self.med_channel_in , self.med_channel_out)
            self.conv_med2 = Conv(self.med_channel_in, self.low_channel_out)
            self.conv_high = Conv(self.big_channel_in, self.low_channel_out)
            self.conv_low = Conv(self.small_channel_in + self.small_channel_in2, self.low_channel_out)
            self.act = nn.Sigmoid()
            # self.cv1 = Conv(2, 1, 3, 1)
            self.act2 = nn.Sigmoid()
            self.pool = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, x):
        if self.in_numbers == 2:
            ...
            med_x1 = self.conv_med1(med_x)
            med_x2 = self.conv_med2(med_x)
            med_x2 = self.act(med_x2)
            low_x = med_x2 * low_x

            high_x_ca = self.act2(self.fc(self.pool(med_x1)))
            high_x_ca = high_x_ca * med_x1
            out = torch.cat([low_x, high_x_ca], 1)
        elif self.in_numbers == 3:
            low_x = x[0]
            med_x = x[1]
            high_x = x[2]
            ...
            high_x = x[2]
            med_x = x[3]
            low_x = torch.cat([small_x, small_x2], 1)
            low_x = self.conv_low(low_x)
            high_x = self.conv_high(high_x)
            med_x1 = self.conv_med1(med_x)
            med_x2 = self.conv_med2(med_x)
            med_x2 = self.act(med_x2)
            high_x = high_x * med_x2
            low_x = low_x * med_x2


            high_x_ca = self.act2(self.fc(self.pool(med_x1)))
            high_x_ca = high_x_ca * med_x1
            out = torch.cat([low_x, high_x_ca, high_x], 1)
            # out = self.out(out)


        return out
class HAC(nn.Module):
    def __init__(self, in_channel, out_channel, all_out):
        super(HAC, self).__init__()
        self.in_numbers = len(in_channel)
        if self.in_numbers == 2:
            self.med_channel_in = in_channel[1]
...
class RepELANMSv2(nn.Module):
    ...
    def forward(self, x):
        ...
        y_out = self.conv2(y_out)
        return y_out
class LACv1(nn.Module):
    def __init__(self, in_channel, out_channel):
        super(LACv1, self).__init__()
        self.in_numbers = len(in_channel)
        if self.in_numbers == 2:
            self.med_channel_in = in_channel[1]
            self.low_channel_in = in_channel[0]
            ...
            self.med_channel_out = out_channel
            self.low_channel_out = out_channel
            self.conv_med1 = Conv(self.high_channel_in, self.med_channel_out)
            self.conv_med2 = Conv(self.high_channel_in, self.low_channel_out)
            self.conv_low = Conv(self.low_channel_in + self.med_channel_in, self.low_channel_out)
            self.act = nn.Sigmoid()
            self.cv1 = Conv(2, 1, 3, 1)
            self.act2 = nn.Sigmoid()
            self.pool = nn.AdaptiveAvgPool2d(1)
...
class LACv2(nn.Module):
    def __init__(self, in_channel, out_channel, reduction =16):
        ...
        if self.in_numbers == 2:
            ...
            ####CA
            self.conv_ca = Conv(self.low_channel_out, self.low_channel_out // reduction)
            self.F_h = nn.Conv2d(self.low_channel_out // reduction, self.low_channel_out, kernel_size=1, stride=1, bias = False)
            self.F_w = nn.Conv2d(self.low_channel_out // reduction, self.low_channel_out, kernel_size=1, stride=1,
                                 bias=False)
    ...
    def forward(self, x):
        if self.in_numbers == 2:
            ...
            ###ca####
            x_h = torch.mean(low_x, dim = 3, keepdim = True).permute(0,1,3,2)
            x_w = torch.mean(low_x, dim = 2, keepdim=True)
            x_cat = self.conv_ca(torch.cat((x_h, x_w), 3))
            x_cat_conv_split_h, x_cat_conv_split_w = x_cat.split([h, w], 3)
            s_h = self.sigmoid_h(self.F_h(x_cat_conv_split_h.permute(0, 1, 3, 2)))
            s_w = self.sigmoid_w(self.F_w(x_cat_conv_split_w))
            low_ca = low_x * s_h.expand_as(low_x) * s_w.expand_as(low_x)

            med_x_ca = self.act2(self.fc(self.pool(med_x1)))
            med_x_ca = med_x_ca * med_x1

            out = low_ca + med_x_ca
            weight = self.act(out)
            out = torch.cat([2 * weight * med_x_ca, weight * low_ca], 1)
            ...
            out = low_ca + med_x_ca
            weight = self.act(out)
            out = torch.cat([weight * med_x_ca, (1-weight) * low_ca], 1)
        ...
        return out
class LowAttentionConcat_up(nn.Module):
    def __init__(self, out_channel, pre_channel, up_channel):
        super(LowAttentionConcat_up, self).__init__()
        self.med_channel = out_channel
        self.low_channel = out_channel // 2
        self.pre_channel = pre_channel
        self.pre_big_channel = pre_channel * 2
        self.conv_med1 = Conv(self.pre_big_channel, self.med_channel, 1, 1)
        self.conv_med2 = Conv(self.pre_big_channel, self.med_channel, 1, 1)
        # self.down = Conv(self.pre_channel, self.low_channel, 3, 2)
        # self.up = nn.Upsample(None, 2, 'nearest')
        self.up_conv = Conv(up_channel, 2 * out_channel, 1, 1)
        self.act = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        return out
class HAC1(nn.Module):
    def __init__(self, in_channel, out_channel):
        super(HAC1, self).__init__()
        self.in_numbers = len(in_channel)
        if self.in_numbers == 2:
            self.med_channel_in = in_channel[1]
            self.low_channel_in = in_channel[0]
            ...
            self.med_channel_in = in_channel[0]
            self.med_channel_in2 = in_channel[1]
            self.big_channel_in = in_channel[2]

            self.med_channel_out = out_channel
            self.low_channel_out = out_channel
            self.conv_med = Conv(self.med_channel_in + self.med_channel_in2, self.med_channel_out)
            self.conv_big1 = Conv(self.big_channel_in, self.low_channel_out)
            self.conv_big2 = Conv(self.big_channel_in, self.low_channel_out)
            ...
            self.med_channel_in2 = in_channel[1]
            self.big_channel_in = in_channel[2]
            self.big_channel_in2 = in_channel[3]
            self.med_channel_out = out_channel
            self.low_channel_out = out_channel
            self.conv_med = Conv(self.med_channel_in + self.med_channel_in2, self.med_channel_out)
            self.conv_big1 = Conv(self.big_channel_in+self.big_channel_in2, self.low_channel_out)
            self.conv_big2 = Conv(self.big_channel_in + self.big_channel_in2, self.low_channel_out)
    ...
    def forward(self, x):
        if self.in_numbers == 2:
            ...
            med_x2 = x[1]
            big_x = x[2]
            med_x = torch.cat([med_x1, med_x2], 1)
            med_x = self.conv_med(med_x)
            big_x1 = self.conv_big1(big_x)
            big_x2 = self.conv_big2(big_x)
            big_x2 = self.act(big_x2)
            med_x = med_x * big_x2

            big_x1_ca = self.act2(self.fc(self.pool(big_x1)))
            big_x1 = big_x1_ca * big_x1
            out = torch.cat([big_x1, med_x], 1)
        elif self.in_numbers == 4:
            med_x1 = x[0]
            med_x2 = x[1]
            big_x1 = x[2]
            ...
            med_x = torch.cat([med_x1, med_x2], 1)
            med_x = self.conv_med(med_x)
            big_x = torch.cat([big_x1, big_x2], 1)
            big_x1 = self.conv_big1(big_x)
            big_x2 = self.conv_big2(big_x)
            big_x2 = self.act(big_x2)
            med_x = med_x * big_x2
        ...
        return out
class LowAttentionConcat_up_old(nn.Module):
    def __init__(self, out_channel, pre_channel, up_channel):
        super(LowAttentionConcat_up, self).__init__()
        self.med_channel = out_channel
        self.low_channel = out_channel // 2
        self.pre_channel = pre_channel
        self.pre_big_channel = pre_channel * 2
        self.conv_med = Conv(self.pre_big_channel, self.med_channel, 1, 1)
        # self.down = Conv(self.pre_channel, self.low_channel, 3, 2)
        # self.up = nn.Upsample(None, 2, 'nearest')
        self.up_conv = Conv(up_channel, 2 * out_channel, 1, 1)
        self.act = nn.Sigmoid()
        ...
        self.act2 = nn.Sigmoid()
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Conv2d(self.med_channel, self.med_channel, 1, 1, 0, bias=True)
    ...
    def forward(self, x):
        ...
        med_x_list = list(med_x.split((self.low_channel, self.low_channel), 1))
        mean_sa = torch.mean(med_x_list[0], 1, keepdim=True)
        max_sa = torch.max(med_x_list[0], 1, keepdim=True)[0]
        med_x_sa = self.act(self.cv1(torch.cat([mean_sa, max_sa], 1)))

        med_x_sa = med_x_sa * low_x
        med_x_sa_out = torch.cat([med_x_list[1], med_x_sa], 1)

        high_x_list = list(high_x.split((self.med_channel, self.med_channel), 1))
        high_x_ca = self.act2(self.fc(self.pool(high_x_list[0])))
        high_x_ca = high_x_ca * med_x_sa_out
        high_x_sa_ca_out = torch.cat([high_x_list[1], high_x_ca], 1)
        # high_x_sa_ca_out = high_x_list[1] + high_x_ca

        return high_x_sa_ca_out
