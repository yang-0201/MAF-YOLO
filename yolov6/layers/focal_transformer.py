...
class FocalTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        # H, W = self.input_resolution
        B, C, H1, W1 = x.shape
        x = x.flatten(2).permute(0, 2, 1)  ###########new

        B, L, C = x.shape

        assert L == H1 * W1, "input feature has wrong size"
        ...
        if self.shift_size > 0:
            ...
            x = shifted_x
        x = x[:, :H1, :W1].contiguous().view(B, -1, C)
        ...
        x = x.view(B, H1, W1, C) ####new
        x = x.permute(0, 3, 1, 2) ####new
        return x
...
class FocalTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.return_idx1 =return_idx -1
        self.patchembed = PatchEmbed(
            img_size=(patches_resolution[0] // (2 ** (self.return_idx1)),patches_resolution[1] // (2 ** self.return_idx1)),
            patch_size=2, in_chans=int(embed_dim * 2 ** self.return_idx1), embed_dim=int(2* embed_dim * 2 ** self.return_idx1),
            use_conv_embed=use_conv_embed, norm_layer=norm_layer, use_pre_norm=use_pre_norm,
            is_stem=False)
        self.input_resolution = (patches_resolution[0] // (2 ** self.return_idx1),patches_resolution[1] // (2 ** self.return_idx1))
    ...
    #     out = []
    #     stride = [4,8,16,32,32]
    #     channels =[self.embed_dim, self.embed_dim * 2, self.embed_dim * 4, self.embed_dim * 8, self.embed_dim * 8]
    #     out.append(x)
    #     for layer in self.layers:
    #         x = layer(x)
    #         out.append(x)
    #     y = out[self.return_idx]
    #     return y

        #
        # if self.return_out:
        #     out_hw_shape = (self.img_size//stride[self.return_idx],self.img_size//stride[self.return_idx])
        #     y = y.view(-1, *out_hw_shape, channels[self.return_idx]).permute(0, 3, 1, 2).contiguous()
        #     return y
        # else:
        #     return y

        # if num == self.return_idx:
        #     out_hw_shape = (self.img_size//stride[num],self.img_size//stride[num])
        #     x = x.view(-1, *out_hw_shape, channels[num]).permute(0, 3, 1, 2).contiguous()

    def forward(self, x):
        if self.return_idx == 0:
            x = self.patch_embed(x)
            if self.ape:
                x = x + self.absolute_pos_embed
            x = self.pos_drop(x)
            return x
        elif self.return_idx == 1:
            x = self.layers[0](x)
            # x = x.view(x.shape[0], self.input_resolution[0], self.input_resolution[1], -1).permute(0, 3, 1,
            #                                                                                        2).contiguous()
            # x = self.patchembed(x)
            ...
            # out_hw_shape = (32,32)
            # x = x.view(-1, *out_hw_shape, 192).permute(0, 3, 1,2).contiguous()
            return x
        elif self.return_idx == 2:
            x = self.layers[1](x)
            # x = x.view(x.shape[0], self.input_resolution[0], self.input_resolution[1], -1).permute(0, 3, 1,
            #                                                                                        2).contiguous()
            # x = self.patchembed(x)
            return x
        elif self.return_idx == 3:
            x = self.layers[2](x)
            # x = x.view(x.shape[0], self.input_resolution[0], self.input_resolution[1], -1).permute(0, 3, 1,
            #                                                                                        2).contiguous()
            # x = self.patchembed(x)
            return x
        elif self.return_idx == 4:
            x = self.layers[3](x)

            # x = self.norm(x)  # B L C
            # out_hw_shape = (20,20)
            # x = x.view(-1, *out_hw_shape, 768).permute(0, 3, 1,2).contiguous()
...
class FocalTransformer_view(nn.Module):
    def __init__(self,imgsize,channel):
        super(FocalTransformer_view, self).__init__()
        # self.idx = idx
    ...
    def forward(self,x):
        # stride = [4, 8, 16, 32, 32]
        # out_hw_shape = (self.imgsize//stride[self.idx],self.imgsize//stride[self.idx])
        out_hw_shape = (self.imgsize,self.imgsize)
        x = x.view(-1, *out_hw_shape, self.channel).permute(0, 3, 1, 2).contiguous()
        return x

class FocalTransformer_block(nn.Module):
    def __init__(self,in_chans, embed_dim,depths=2,
                 num_heads=16,img_size = 20,patch_size = 1,
                 window_size=7,
                 mlp_ratio=4.,
                 qkv_bias=True,
                 qk_scale=None,
                 ...
                 use_checkpoint=False,
                 use_shift=False,
                 focal_levels=2,  #[2, 2, 2, 2]
                 focal_windows=7, #[7, 5, 3, 1]
                 focal_pool="fc",
                 expand_sizes=3,
                 expand_layer="all",
                 use_conv_embed=True,
                 ...
                 layerscale_value=1e-4,
                 use_pre_norm=False,):
        super(FocalTransformer_block, self).__init__()
        self.img_size = img_size
        # self.patch1 = PatchEmbed((img_size,img_size), patch_size, embed_dim, embed_dim)
        self.layers = nn.ModuleList()
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum([depths]))]  # stochastic depth decay rule
        self.layers = BasicLayer(dim = int(embed_dim),
                           input_resolution=(int(img_size/patch_size), int(img_size/patch_size)),
                           depth=depths,
                           num_heads=num_heads,
                           window_size=window_size,
                           mlp_ratio=mlp_ratio,
        ...
        self.focalTransformer_view = FocalTransformer_view(int(img_size/patch_size),embed_dim)
        self.embed_dim = embed_dim
        #no patch embed
        self.conv = None
        if in_chans != embed_dim:
            self.conv = Conv1(in_chans, embed_dim)
        self.linear = nn.Linear(embed_dim, embed_dim)  # learnable position embedding
    def forward(self,x):
        b, _, w, h = x.shape
        if self.conv is not None:
            x = self.conv(x)
        # x1 = self.patch1(x)
        ######x = x.flatten(2).permute(0, 2, 1)  放进layer！
        # x = x.flatten(2).permute(0, 2, 1)
        x = self.layers(x)
        ###focalTransformer_view
        out_hw_shape = (w,h)
        x = x.view(-1, *out_hw_shape, self.embed_dim).permute(0, 3, 1, 2).contiguous()
        # x = self.focalTransformer_view(x)
        # x = self.layers(p + self.linear(p)).permute(1, 2, 0).reshape(b, self.c2, w, h)
        # x = self.patch1(x)
        # x = self.layers(x)
        # x = self.focalTransformer_view(x)
        return x
