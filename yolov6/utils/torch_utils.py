...
def fuse_model(model):
    ...
    from yolov6.layers.common import Conv, SimConv, Conv_C3, ConvWrapper, DWConv

    for m in model.modules():
        if (type(m) is Conv or type(m) is SimConv or type(m) is Conv_C3 or type(m) is ConvWrapper) and hasattr(m, "bn"):
            m.conv = fuse_conv_and_bn(m.conv, m.bn)  # update conv
            delattr(m, "bn")  # remove batchnorm
            m.forward = m.forward_fuse  # update forward
        if (type(m) is DWConv) and hasattr(m, "bn"):
            m.conv = fuse_conv_and_bn(m.conv, m.bn)  # update conv
            delattr(m, "bn")  # remove batchnorm
            m.forward = m.forward_fuse  # update forward
    return model
