...
class Evaler:
    ...
    def init_model1(self, model,cfg, weights, task):
        if task != 'train':
            from yolov6.models.yolo import build_model
            model = build_model(cfg, 80, 0, 640)
            ...
            from yolov6.layers.common import ReparamLargeKernelConv
            for layer in model.modules():
                if isinstance(layer, RepVGGBlock):
                    layer.switch_to_deploy()
                    LOGGER.info("Switch model to ReparamLargeKernelConv")
                if isinstance(layer, ReparamLargeKernelConv):
                    layer.merge_kernel()
                    LOGGER.info("Switch model to ReparamLargeKernelConv")
    ...
    def init_model(self, model, weights, task):
        if task != 'train':
            ...
            # switch to deploy
            from yolov6.layers.common import RepVGGBlock
            from yolov6.layers.common import ReparamLargeKernelConv, UniRepLKNetBlock
            ...
            for layer in model.modules():
                ...
                if isinstance(layer, ReparamLargeKernelConv):
                    layer.merge_kernel()
                    LOGGER.info("Switch model to ReparamLargeKernelConv")
                if isinstance(layer, UniRepLKNetBlock):
    ...
    def init_data(self, dataloader, task):
        ...
        if task != 'train':
            ...
            dataloader = create_dataloader(self.data[task if task in ('train', 'val', 'test') else 'val'],
                                           self.img_size, self.batch_size, 32, hyp=eval_hyp, check_labels=True, pad=pad, rect=rect,
    ...
    def eval_model(self, pred_results, model, dataloader, task):
        ...
        if task != 'speed' and len(pred_results):
            ...
            cocoEval.summarize()
            map, map50, map75, map_s, map_m, map_l = cocoEval.stats[:6]  # update results (mAP@0.5:0.95, mAP@0.5)
            # Return results
            model.float()  # for training
            if task != 'train':
                LOGGER.info(f"Results saved to {self.save_dir}")
            return (map50, map, map75, map_s, map_m, map_l)
        return (0.0, 0.0)
