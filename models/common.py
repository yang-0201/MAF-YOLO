...
class Input(nn.Module):
    ...
    def forward(self, x):
        return x
class CBSwinTransformer_out(nn.Module):
    # Standard convolution
    def __init__(self):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
...
class MPC(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
class DecoupledHead_yolov6(nn.Module):
    ...
    def forward(self, x):
        ...
        x21 = self.conv1(x2)
        x22 = self.conv1(x2)
        x31 = self.reg_preds(x21)
        x32 = self.obj_preds(x22)

        out = torch.cat([x31, x32, x1], 1)
        return out
...
class ERB(nn.Module):
    def __init__(self, in_channels, out_channels,c1 = 512):
        ...
        self.sobel_x1, self.sobel_y1 = get_sobel(c1, 1)
        self.c1 = c1

    def forward(self, x, relu=True):
        x = run_sobel(self.sobel_x1, self.sobel_y1, x)
        x = self.conv1(x)
        res = self.conv2(x)
        res = self.bn(res)
        res = self.relu(res)
...
class MP1(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.idx == 1:
            out = torch.cat([x1, x2], dim=1)
        return out
class ERB_down(nn.Module):
    def __init__(self, in_channels, out_channels,c1 = 512):
        super(ERB_down, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0)
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.relu = nn.ReLU()
        self.bn = nn.BatchNorm2d(out_channels)
        ...
        self.c1 = c1
        # self.mp = MP1(out_channels,1)
        self.conv4 = Conv(in_channels,c1,3,2)
    def forward(self, x, relu=True):
        x1 = run_sobel(self.sobel_x1, self.sobel_y1, x)
        x1 = self.conv1(x1)
        res = self.conv2(x1)
        res = self.bn(res)
        res = self.relu(res)
